...
const Podcast = () => {
  ...
  const [motivationVideos, setMotivationVideos] = useState([]);
  const [interviewVideos, setInterviewVideos] = useState([]);
  const [videoVideos, setVideoVideos] = useState([]);
  const [filteredOtherVideos, setFilteredOtherVideos] = useState([]);
  const [filteredMotivationVideos, setFilteredMotivationVideos] = useState([]);
  const [filteredInterviewVideos, setFilteredInterviewVideos] = useState([]);
  const [filteredVideoVideos, setFilteredVideoVideos] = useState([]);
  ...
  useEffect(() => {
    ...
    fetchVideos('Other', setOtherVideos);
    fetchVideos('Motivation', setMotivationVideos);
    fetchVideos('Interview', setInterviewVideos);
    fetchVideos('Video', setVideoVideos);
  }, []);
  ...
  const applyFilter = useCallback(
    (category, filter, videosToFilter) => {
      ...
      if (category === 'Other') {
        setFilteredOtherVideos(sortedVideos);
      } else if (category === 'Motivation') {
        setFilteredMotivationVideos(sortedVideos);
      } else if (category === 'Interview') {
        setFilteredInterviewVideos(sortedVideos);
      } else if (category === 'Video') {
        setFilteredVideoVideos(sortedVideos);
      }
    },
    []
  );
  ...
  useEffect(() => {
    applyFilter('Other', activeFilters.Other, otherVideos);
  }, [activeFilters.Other, otherVideos, applyFilter]);

  useEffect(() => {
    applyFilter('Motivation', activeFilters.Motivation, motivationVideos);
  }, [activeFilters.Motivation, motivationVideos, applyFilter]);

  useEffect(() => {
    applyFilter('Interview', activeFilters.Interview, interviewVideos);
  }, [activeFilters.Interview, interviewVideos, applyFilter]);

  useEffect(() => {
    applyFilter('Video', activeFilters.Video, videoVideos);
  }, [activeFilters.Video, videoVideos, applyFilter]);
  ...
  if (
    ...
    errorCategories.Motivation &&
    errorCategories.Interview &&
    errorCategories.Video &&
    !loadingCategories.Other &&
    !loadingCategories.Motivation &&
    !loadingCategories.Interview &&
    !loadingCategories.Video
  ) {
    return (
      <div className="flex flex-col items-center justify-center h-screen text-gray-800 p-4">
        ...
        <p className="mb-6 text-gray-700 text-center">
          {errorCategories.Other === errorCategories.Motivation &&
          errorCategories.Motivation === errorCategories.Interview &&
          errorCategories.Interview === errorCategories.Video
            ? errorCategories.Other
            : `${errorCategories.Other} (Other) | ${errorCategories.Motivation} (Motivation) | ${errorCategories.Interview} (Interview) | ${errorCategories.Video} (Video)`}
        </p>
        <button
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 p-6 md:p-8">
      <div className="max-w-7xl mx-auto">
        <div className="relative inline-block mb-12">
          <h1 className="text-4xl md:text-5xl font-bold relative z-10">Podcast</h1>
          <div
        ...
        <div className="mb-10 pb-6 border-b border-gray-100">
          <div className="flex flex-wrap gap-4">
            {['All', 'Other', 'Motivation', 'Interview', 'Video'].map((tab) => (
              <Link
                key={tab}
                to={tabRoutes[tab]}
        ...
        <CategorySection
          videos={filteredOtherVideos}
          category="Other"
          path="other"
          isLoading={loadingCategories.Other}
          error={errorCategories.Other}
        />
        <CategorySection
          videos={filteredMotivationVideos}
          category="Motivation"
          path="motivation"
          isLoading={loadingCategories.Motivation}
          error={errorCategories.Motivation}
        />
        <CategorySection
          videos={filteredInterviewVideos}
          category="Interview"
          path="interviews"
          isLoading={loadingCategories.Interview}
          error={errorCategories.Interview}
        />
        <CategorySection
          videos={filteredVideoVideos}
          category="Video"
          path="videos"
          isLoading={loadingCategories.Video}
          error={errorCategories.Video}
        />
      </div>
    </div>
  );
