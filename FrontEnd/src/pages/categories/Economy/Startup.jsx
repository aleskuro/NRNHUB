    ...
    return (
      <div className={`w-full bg-gray-100 rounded-xl overflow-hidden shadow-md flex items-center justify-center py-8 ${className || ''}`}>
        <div className="text-center p-6">
          ...
          <a href="/advertise" className="mt-4 inline-block text-[#C2410C] hover:text-[#A53605] text-sm font-medium">Learn More →</a>
        </div>
      </div>
    );
  }
...
const BlogCard = ({ blog, truncateTitle, getReadTime, getAuthorName, getCategoryColor, formatDate }) => (
  ...
  >
    <div className="bg-gradient-to-br from-white to-orange-50 rounded-xl shadow-sm overflow-hidden mb-6 hover:shadow-md transition-shadow border border-orange-100">
      ...
      <div className="p-5">
        <h3 className="text-xl font-semibold text-gray-800 mb-3 group-hover:text-[#C2410C] transition-colors">
        ...
        <div className="flex items-center justify-between pt-3 border-t border-gray-100">
          ...
          <div className="flex items-center">
            ...
            <div className="flex items-center text-sm">
              <Share2 size={14} className="mr-1 text-orange-500" />
...
const Startup = () => {
  ...
  const [activeFilter, setActiveFilter] = useState('STARTUP');
  ...
  // Fetch ads on mount
  useEffect(() => {
    console.log('Fetching ads for Startup page...');
  ...
  // Debug ad state
  useEffect(() => {
    console.log('Current ads state for Startup:', {
      adImages,
      adLinks,
      visibility,
      keyAds: {
  ...
  const { startupBlogs, featuredBlog, suggestedBlogs } = useMemo(() => {
    const filteredStartupBlogs = blogs
      .filter((blog) => blog.category?.toUpperCase() === 'STARTUP')
      .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

    const startupBlogIds = new Set(filteredStartupBlogs.map((blog) => blog._id));
    const otherBlogs = blogs
      .filter((blog) => !startupBlogIds.has(blog._id) && blog.category?.toUpperCase() !== 'STARTUP')
      .sort(() => Math.random() - 0.5)
      .slice(0, 3);

    return {
      startupBlogs: filteredStartupBlogs,
      featuredBlog: filteredStartupBlogs[0],
  ...
  return (
    <div className="bg-gray-50 min-h-screen text-gray-800">
      <Helmet>
        <link rel="canonical" href="http://localhost:5173/startup" />
        <title>Startup Blogs | NRNHUB</title>
        <meta
          name="description"
          content="Fuel your entrepreneurial journey with insights, strategies, and stories from the startup world, covering innovation, funding, and scaling."
        />
      </Helmet>

      {/* Hero Banner Section */}
      <div className="relative py-20 bg-gradient-to-r from-[#C2410C] to-[#F97316] overflow-hidden rounded-b-2xl">
          ...
          <div className="md:flex items-center justify-between gap-12">
            <div className="md:w-7/12 mb-10 md:mb-0 text-center md:text-left">
              ...
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-4 text-white leading-tight">
                Startup <span className="text-orange-200">Blog</span>
              </h1>
              <p className="text-lg md:text-xl text-orange-100 max-w-xl mx-auto md:mx-0 leading-relaxed">
                Fuel your entrepreneurial journey with insights, strategies, and stories from the startup world. Explore
                tips on innovation, funding, scaling, and building a business that disrupts the status quo.
              </p>
            </div>
            <div className="md:w-5/12 flex justify-center md:justify-end">
              {isLoading && (
                <div className="w-full max-w-sm h-64 bg-orange-300/30 rounded-2xl animate-pulse flex items-center justify-center">
                  <span className="text-orange-200">Loading Featured...</span>
                </div>
              )}
              {!isLoading && !featuredBlog && (
                <div className="w-full max-w-sm h-64 bg-orange-300/30 rounded-2xl flex items-center justify-center">
                  <span className="text-orange-200 text-center p-4">No featured article available.</span>
                </div>
              )}
              {!isLoading && featuredBlog && (
                <div className="relative w-full max-w-sm">
                  <div className="absolute -top-4 -left-4 w-24 h-24 rounded-lg border-2 border-orange-300/30 hidden md:block"></div>
                  <FeaturedBlogCard
                    blog={featuredBlog}
          ...
          <div className="lg:w-2/3">
            {isLoading && (
              <div className="text-center py-12">
                <div className="animate-spin rounded-full h-12 w-12 border-t-4 border-b-4 border-[#C2410C] mx-auto"></div>
            ...
            {!isLoading && !error && startupBlogs.length === 0 && (
              <div className="text-center py-12 bg-white rounded-xl p-6 shadow-sm border border-gray-200">
                ...
                <p className="text-gray-500">Check back later for new startup content.</p>
              </div>
            )}
            {!isLoading && !error && startupBlogs.length > 0 && (
            ...
            <div className="sticky top-6 space-y-8">
              <div>
                <h2 className="text-xl font-bold text-gray-800 mb-4 flex items-center">
                  <span className="bg-[#C2410C] w-2 h-6 mr-2 rounded-sm inline-block"></span>
                  Sponsored Content
                </h2>
              ...
              <div className="bg-gradient-to-r from-[#C2410C] to-[#F97316] rounded-xl shadow-sm p-6 text-white">
                <h3 className="text-lg font-bold mb-2">Join Our Newsletter</h3>
                <p className="text-sm mb-4 text-orange-100">
                  Subscribe to receive the latest startup insights directly in your inbox.
                </p>
                <form onSubmit={handleSubscribe} className="flex flex-col gap-3" aria-label="Newsletter Subscription">
                  <div className="relative">
                    <input
                      ...
                      } focus:outline-none focus:ring-2 focus:ring-orange-300 placeholder-gray-400`}
                  ...
                  <button
                    type="submit"
                    className="bg-white text-[#C2410C] px-4 py-2 rounded-lg font-medium hover:bg-gray-100 transition-colors border-2 border-white focus:outline-none focus:ring-2 focus:ring-orange-300 disabled:opacity-50"
                    disabled={isSubmitting}
        ...
        <div className="container mx-auto px-4">
          <div className="text-center max-w-3xl mx-auto">
            ...
            <div className="flex justify-center gap-4">
              <Link
                to="/smallbiz"
                className="bg-[#C2410C] text-white px-6 py-3 rounded-lg font-medium hover:bg-[#A53605] transition-colors shadow-md focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-opacity-50"
                onClick={() => window.scrollTo(0, 0)}
              >
