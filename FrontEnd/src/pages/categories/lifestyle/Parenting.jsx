...
const Parenting = () => {
  ...
  const [activeFilter, setActiveFilter] = useState('Parenting');
  ...
  // Fetch ads on mount
  useEffect(() => {
    console.log('Fetching ads for Parenting page...');
  ...
  // Debug ad state
  useEffect(() => {
    console.log('Current ads state for Parenting:', {
      adImages,
      adLinks,
      visibility,
      keyAds: {
  ...
  const { ParentingBlogs, featuredBlog, suggestedBlogs } = useMemo(() => {
    const filteredParentingBlogs = blogs
      .filter((blog) => {
        const blogCategory = blog.category?.toUpperCase();
        const isMatch = blogCategory === 'PARENTING';
        if (!isMatch && blog.category) {
          console.log(`Blog excluded: ${blog.title}, Category: ${blog.category} (Expected: Parenting)`);
        }
        return isMatch;
      })
      .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

    const ParentingBlogIds = new Set(filteredParentingBlogs.map((blog) => blog._id));
    const otherBlogs = blogs
      .filter((blog) => !ParentingBlogIds.has(blog._id) && blog.category?.toUpperCase() !== 'PARENTING')
      .sort(() => Math.random() - 0.5)
      .slice(0, 3);

    console.log('Filtered Parenting Blogs:', filteredParentingBlogs);
    console.log('Featured Blog:', filteredParentingBlogs[0]);
    console.log('Suggested Blogs:', otherBlogs);

    return {
      ParentingBlogs: filteredParentingBlogs,
      featuredBlog: filteredParentingBlogs[0],
  ...
  return (
    <div className="bg-gray-50 min-h-screen text-gray-800">
      <Helmet>
        <link rel="canonical" href="http://localhost:5173/parenting" />
        <title>Parenting Blogs | NRNHUB</title>
        <meta
          name="description"
          content="Explore our Parenting blog for insightful articles, tips, and trends on raising children, family life, and more."
        />
      </Helmet>
          ...
          <div className="md:flex items-center justify-between gap-12">
            <div className="md:w-7/12 mb-10 md:mb-0 text-center md:text-left">
              ...
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-4 text-white leading-tight">
                Parenting <span className="text-purple-200">Blog</span>
              </h1>
              <p className="text-lg md:text-xl text-purple-100 max-w-xl mx-auto md:mx-0 leading-relaxed">
                Explore our Parenting blog for insightful articles, tips, and trends on raising children, family life, and more.
              </p>
            </div>
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-2/3">
            ...
            {!isLoading && !error && ParentingBlogs.length === 0 && (
              <div className="text-center py-12 bg-white rounded-xl p-6 shadow-sm border border-gray-200">
                ...
                <p className="text-gray-500">Check back later for new Parenting content.</p>
              </div>
            )}
            {!isLoading && !error && ParentingBlogs.length > 0 && (
              <div className="grid md:grid-cols-2 gap-6">
                {ParentingBlogs.map((blog) => (
                  <BlogCard
            ...
            <div className="sticky top-6 space-y-8">
              ...
              <div className="bg-gradient-to-r from-[#7734E3] to-[#5E30B2] rounded-xl shadow-sm p-6 text-white">
                ...
                <p className="text-sm mb-4 text-purple-100">
                  Subscribe to receive the latest Parenting updates directly in your inbox.
                </p>
