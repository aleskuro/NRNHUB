...
const Health = () => {
  ...
  // Fetch ads on mount
  useEffect(() => {
    console.log('Fetching ads for Health page...');
  ...
  // Debug ad state
  useEffect(() => {
    console.log('Current ads state for Health:', {
      adImages,
      adLinks,
      visibility,
      keyAds: {
  ...
  const { healthBlogs, featuredBlog, suggestedBlogs } = useMemo(() => {
    const filteredHealthBlogs = blogs
      .filter((blog) => blog.category?.toUpperCase() === 'HEALTH')
      .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

    const healthBlogIds = new Set(filteredHealthBlogs.map((blog) => blog._id));
    const otherBlogs = blogs
      .filter((blog) => !healthBlogIds.has(blog._id) && blog.category?.toUpperCase() !== 'HEALTH')
      .sort(() => Math.random() - 0.5)
      .slice(0, 3);

    return {
      healthBlogs: filteredHealthBlogs,
      featuredBlog: filteredHealthBlogs[0],
  ...
  return (
    <div className="bg-gray-50 min-h-screen text-gray-800">
      <Helmet>
        <link rel="canonical" href="http://localhost:5173/health" />
        <title>Health Blogs | NRNHUB</title>
        <meta
          name="description"
          content="Empower your well-being with our health blog, offering wellness, fitness, and mental health insights, expert tips, and practical advice."
        />
      </Helmet>
            ...
            <div className="sticky top-6 space-y-8">
              ...
              <div className="bg-gradient-to-r from-[#7734E3] to-[#5E30B2] rounded-xl shadow-sm p-6 text-white">
                ...
                <p className="text-sm mb-4 text-purple-100">
                  Subscribe to receive the latest health and wellness tips directly in your inbox.
                </p>
