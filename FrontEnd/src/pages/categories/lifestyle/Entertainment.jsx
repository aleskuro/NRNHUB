...
// FeaturedBlogCard Component (Fixed)
...
const Entertainment = () => {
  ...
  // Fetch ads on mount
  useEffect(() => {
    console.log('Fetching ads for Entertainment page...');
  ...
  // Debug ad state
  useEffect(() => {
    console.log('Current ads state for Entertainment:', {
      adImages,
      adLinks,
      visibility,
      keyAds: {
  ...
  const { entertainmentBlogs, featuredBlog, suggestedBlogs } = useMemo(() => {
    const filteredEntertainmentBlogs = blogs
      .filter((blog) => {
        const blogCategory = blog.category?.toUpperCase();
        const isMatch = blogCategory === 'ENTERTAINMENT';
        if (!isMatch && blog.category) {
          console.log(`Blog excluded: ${blog.title}, Category: ${blog.category} (Expected: ENTERTAINMENT)`);
        }
        return isMatch;
      })
      .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

    const entertainmentBlogIds = new Set(filteredEntertainmentBlogs.map((blog) => blog._id));
    const otherBlogs = blogs
      .filter((blog) => !entertainmentBlogIds.has(blog._id) && blog.category?.toUpperCase() !== 'ENTERTAINMENT')
      .sort(() => Math.random() - 0.5)
      .slice(0, 3);

    console.log('Filtered Entertainment Blogs:', filteredEntertainmentBlogs);
    console.log('Featured Blog:', filteredEntertainmentBlogs[0]);
    console.log('Suggested Blogs:', otherBlogs);

    return {
      entertainmentBlogs: filteredEntertainmentBlogs,
      featuredBlog: filteredEntertainmentBlogs[0],
  ...
  return (
    <div className="bg-gray-50 min-h-screen text-gray-800">
      <Helmet>
        <link rel="canonical" href="http://localhost:5173/entertainment" />
        <title>Entertainment Blogs | NRNHUB</title>
        <meta
          name="description"
          content="Step into the dazzling world of entertainment with our blog, featuring movies, music, TV, and pop culture with captivating stories and trends."
        />
      </Helmet>
          ...
          <div className="md:flex items-center justify-between gap-12">
            <div className="md:w-7/12 mb-10 md:mb-0 text-center md:text-left">
              ...
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-4 text-white leading-tight">
                Entertainment <span className="text-purple-200">Blog</span>
              </h1>
              <p className="text-lg md:text-xl text-purple-100 max-w-xl mx-auto md:mx-0 leading-relaxed">
                Step into the dazzling world of entertainment, where movies, music, TV, and pop culture come alive. Discover captivating stories, celebrity insights, and the latest trends that keep you hooked and inspired.
              </p>
            </div>
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-2/3">
            ...
            {!isLoading && !error && entertainmentBlogs.length === 0 && (
              <div className="text-center py-12 bg-white rounded-xl p-6 shadow-sm border border-gray-200">
                ...
                <p className="text-gray-500">Check back later for new entertainment content.</p>
              </div>
            )}
            {!isLoading && !error && entertainmentBlogs.length > 0 && (
            ...
            <div className="sticky top-6 space-y-8">
              ...
              <div className="bg-gradient-to-r from-[#7734E3] to-[#5E30B2] rounded-xl shadow-sm p-6 text-white">
                ...
                <p className="text-sm mb-4 text-purple-100">
                  Subscribe to receive the latest entertainment updates directly in your inbox.
                </p>
