...
const Sports = () => {
  ...
  // Fetch ads on mount
  useEffect(() => {
    console.log('Fetching ads for Sports page...');
  ...
  // Debug ad state
  useEffect(() => {
    console.log('Current ads state for Sports:', {
      adImages,
      adLinks,
      visibility,
      keyAds: {
        sports1: { image: adImages?.sports1, link: adLinks?.sports1, visible: visibility?.sports1 },
        sports2: { image: adImages?.sports2, link: adLinks?.sports2, visible: visibility?.sports2 },
      },
    });
  }, [adImages, adLinks, visibility]);

  // Filter and sort sports blogs, and get suggested blogs
  const { sportsBlogs, featuredBlog, suggestedBlogs } = useMemo(() => {
    const filteredSportsBlogs = blogs
      .filter((blog) => blog.category?.toUpperCase() === 'SPORTS')
      .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

    const sportsBlogIds = new Set(filteredSportsBlogs.map((blog) => blog._id));
    const otherBlogs = blogs
      .filter((blog) => !sportsBlogIds.has(blog._id) && blog.category?.toUpperCase() !== 'SPORTS')
      .sort(() => Math.random() - 0.5)
      .slice(0, 3);

    return {
      sportsBlogs: filteredSportsBlogs,
      featuredBlog: filteredSportsBlogs[0],
  ...
  return (
    <div className="bg-gradient-to-b from-[#E6F0FA] to-[#F3E8FF] min-h-screen text-gray-800">
      <Helmet>
        <link rel="canonical" href="http://localhost:5173/sports" />
        <title>Sports Blogs | NRNHUB</title>
        <meta
          name="description"
          content="Feel the thrill of the game with our sports blog, featuring athletic triumphs, team stories, and fitness tips."
        />
      </Helmet>
          ...
          <div className="md:flex items-center justify-between gap-12">
            <div className="md:w-7/12 mb-10 md:mb-0 text-center md:text-left">
              ...
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-4 text-white leading-tight">
                Sports <span className="text-purple-200">Blogs</span>
              </h1>
              <p className="text-lg md:text-xl text-purple-100 max-w-xl mx-auto md:mx-0 leading-relaxed">
                Feel the thrill of the game with our sports blog, where athletic triumphs, team stories, and fitness tips ignite your passion.
              </p>
            </div>
          ...
          <div className="lg:w-2/3">
            {isLoading && (
              <div className="text-center py-16">
                ...
                <p className="mt-4 text-lg text-gray-600">Loading sports blogs...</p>
              </div>
            )}
            {error && (
            ...
            {!isLoading && !error && sportsBlogs.length === 0 && (
              <div className="text-center py-16">
                <div className="w-24 h-24 bg-gray-200 rounded-full flex items-center justify-center mx-auto mb-6">
                  <BookOpen size={40} className="text-gray-400" />
                </div>
                <h3 className="text-2xl font-bold text-gray-700 mb-2">No blogs found</h3>
                <p className="text-gray-600 mb-6">We couldn't find any blogs in the sports category.</p>
                <Link
                  to="/blogs"
          ...
          <div className="lg:w-1/3">
            <div className="sticky top-6 space-y-8">
              <div>
                ...
                <AdSpace adType="sports1" positionLabel="First" className="mb-6 w-full" />
                <AdSpace adType="sports2" positionLabel="Second" className="mb-6 w-full" />
              </div>
              ...
              <div className="bg-gradient-to-r from-[#7734E3] to-[#5E30B2] rounded-xl shadow-sm p-6 text-white">
                ...
                <p className="text-sm mb-4 text-purple-100">
                  Get the latest sports news and fitness tips delivered to your inbox.
                </p>
