...
const UpdatePost = () => {
  ...
  const getAuthorId = (user) => {
    if (!user || !user._id) {
      console.warn("User or user._id is undefined, falling back to null");
      return null;
    }
    return user._id;
  };
  ...
  useEffect(() => {
    if (blog && !isFetching) {
      ...
      setRating(blog.rating || 0);
      setEditorContent(blog.content?.data || "");
      setConclusionContent(blog.conclusion?.data || "");
      setShowConclusion(!!blog.conclusion?.data);
    }
  }, [blog, isFetching]);
  ...
  useEffect(() => {
    ...
    if (isError) {
      toast.error(`Failed to update blog post: ${updateError?.data?.message || "Something went wrong"}`);
    }
  }, [isSuccess, isError, updateError, navigate, resetUpdate]);
  ...
  const selectCategory = (selectedCategory) => {
    ...
    setShowCategorySuggestions(false); // Hide the dropdown
  };
  ...
  if (isFetching) {
    return <div className="text-center py-4 text-gray-600 font-[Lato]">Loading...</div>;
  }

  if (!blog) {
    return <div className="text-center py-4 text-gray-600 font-[Lato]">Blog not found.</div>;
  }
          ...
          <div className="md:w-2/3 w-full">
            ...
            <div className="mb-8">
              ...
              {showConclusion && (
                <>
                  <p className="text-sm italic text-gray-500 mb-2 font-[Lato]">
                    Update your conclusion here (optional)...
                  </p>
                  <div className="bg-gray-100 rounded-md border border-gray-300">
                    <ReactQuill
              ...
              {showCategorySuggestions && (
                <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg max-h-60 overflow-auto">
                  {categoryOptions
                    .filter(
                      (opt) =>
                        opt.toLowerCase().includes(category.toLowerCase()) || category === ""
                    )
                    .map((option, index) => (
                      <div
                        key={index}
